...
class BetterEmbed extends EmbedBuilder {
    ...
    async send(options) {
        ...
        // Change the embed's footer if applicable
        if (options.footer.text) this.setFooter({ text: options.footer.text, iconURL: options.footer.iconURL });
        if (options.footer.iconURL) this.setFooter({
            text: this.data.footer.text, iconURL: options.footer.iconURL
        });
...
class EmbedNavigator {
    ...
    constructor(options) {
        ...
        this.data = {
            ...
            timeout: dateTools.parseStr(options.timeout),
...
class ac_options {
    constructor() {
         ...
         * @example
         // Text formatting shorthand:
         * "%AUTHOR_NAME" = "the author's display/user name" */
        this.author_text = "Please confirm this action";
        this.description = "";
        this.footer_text = "";

        this.showAuthorIcon = true;
         ...
         * 
         * @type {"reply" | "editReply" | "followUp" | "send"} */
        this.method = "followUp";

        this.deleteOnConfirm = true;
        this.timeout = dateTools.parseStr(timeouts.confirmation);
    }
}

/** Send a confirmation embed and await the user's answer.
 * @param {ac_options} options */
async function awaitConfirmation(options) {
    options = { ...new ac_options(), ...options };
    options.timeout = dateTools.parseStr(options.timeout); let confirmed = false;

    // Format the embed title
    options.author_text = options.author_text
        .replace("%AUTHOR_NAME", options.interaction.member.displayName);

    // Create the embed
    let embed_confirmation = new BetterEmbed({
        interaction: options.interaction, author: {
            text: options.author_text,
            iconURL: options.showAuthorIcon ? "" : null,
            user: options.interaction.member
        },
        description: options.description,
        footer: { text: options.footer_text }
    });
    ...
    let actionRow = new ActionRowBuilder().addComponents(buttons.confirm, buttons.cancel);

    // Send the confirmation embed
    let message = await embed_confirmation.send({ method: options.method, components: actionRow });
    ...
    return new Promise(resolve => {
        ...
        message.awaitMessageComponent({ filter, time: options.timeout }).then(async i => {
            ...
            if ((options.deleteOnConfirm && confirmed) || !confirmed) message.delete();
...
module.exports = {
    BetterEmbed, EmbedNavigator,
    awaitConfirmation, deleteMesssageAfter,
