...
function quest_getProgress(questID, userData, questCache) {
	...
	for (let obj of Object.keys(quest.objectives))
		switch (obj) {
			case "balance":
				objectives.balance = quest.objectives?.balance <= questCache.balance;
				break;
			case "ribbons":
				objectives.ribbons = quest.objectives?.ribbons <= questCache.ribbons;
				break;
			case "cards_in_inventory":
				objectives.cards_in_inventory =
					quest.objectives?.cards_in_inventory <= questCache.cards_in_inventory;
				break;

			case "level_user":
				objectives.level_user = quest.objectives?.level_user <= questCache.level_user;
				break;
			case "level_idol":
				objectives.level_idol = quest.objectives?.level_idol <= questCache.level_idol;
				break;

			case "team_ability_reputation":
				objectives.team_ability_reputation =
					quest.objectives?.team_ability_reputation <=
					(questCache.team_ability && questCache.team_reputation);
				break;

			case "card_global_ids":
				objectives.card_global_ids = userParser.cards.has(
					userData,
					quest.objectives.card_global_ids
				);
				break;
			case "card_sets_complete":
				objectives.card_sets_complete = userParser.cards.setsCompleted(
					userData,
					quest.objectives.card_sets_complete
				);
				break;
			case "card_duplicates":
				objectives.card_duplicates =
					quest.objectives.card_duplicates
						.map(d => userParser.cards.hasDuplicates(userData, d.globalID, d.count))
						.filter(b => b).length === quest.objectives.card_duplicates.length;
				break;
		}
	...
	let objectives_complete = Object.entries(objectives)
		.filter(obj => obj[1])
		.map(obj => obj[0]);
	let objectives_required = Object.entries(objectives)
		.filter(obj => !obj[1])
		.map(obj => obj[0]);
...
function quest_toString_objectiveDescription(questID, objectiveType) {
	...
	switch (objectiveType) {
		case "balance":
			return `\`🥕 Balance\` - \`get ${quest.objectives?.balance || "n/a"} new ${
				quest.objectives?.balance === 1 ? "carrot" : "carrots"
			}\``;
		case "ribbons":
			return `\`🎀 Ribbons\` - \`get ${quest.objectives?.ribbons || "n/a"} new ${
				quest.objectives?.ribbons === 1 ? "ribbon" : "ribbons"
			}\``;
		case "cards_in_inventory":
			return `\`🃏 Inventory\` - \`drop ${
				quest.objectives?.cards_in_inventory || "n/a"
			} new ${quest.objectives?.cards_in_inventory === 1 ? "card" : "cards"}\``;
		case "level_user":
			return `\`📈 User LV.\` - \`reach LV. ${quest.objectives?.level_user || "n/a"}\``;
		case "level_idol":
			return `\`📈 Idol LV.\` - \`reach LV. ${quest.objectives?.level_idol || "n/a"}\``;
		case "team_ability_reputation":
			return `\`👯‍♀️ ABI REP\` - \`reach ${
				quest.objectives?.team_ability_reputation || "n/a"
			} in ABI. REP. stats\``;
		case "card_global_ids":
			return `\`🃏 GID\` - \`own ${
				quest.objectives?.card_global_ids?.length === 1 ? "a card" : "cards"
			} with ${quest.objectives?.card_global_ids?.map(gid => `gid ${gid}`).join(", ")}\``;
		case "card_sets_complete":
			return `\`🗣️ Set\` - \`complete ${
				quest.objectives?.card_sets_complete?.length === 1 ? "set" : "sets"
			} ${quest.objectives?.card_sets_complete?.join(", ")}\``;
		case "card_duplicates":
			return `\`🃏 Dupes\` - \`own ${quest.objectives?.card_duplicates
				?.map(d => `${d.count} dupes of gid ${d.globalID}`)
				.join(", ")}\``;
...
async function mongo_user_markQuestComplete(userID, questID) {
	...
	let cards_reward =
		quest.rewards?.card_global_ids?.map(globalID => cardManager.get.byGlobalID(globalID)) || [];
	// Fillter out nulls
...
async function mongo_user_questsChanged(userID) {
	// Fetch user quest cache from Mongo
    let questCache = await mongo_questCache_fetch(userID);

	return (
		questCache.quests_in_progress.filter(questID => quest_ids.includes(questID)).length !==
		quest_ids.length
	);
}
...
async function mongo_questCache_updateCache(userID) {
	...
	if (await mongo_user_isQuestComplete(userID, quest_ids)) {
		// Reset quest cache
		await mongo_questCache_reset(userID);
		return;
	}
	...
	let userActiveQuests = quests.filter(
		quest => !userData.quests_complete.map(({ id }) => id).includes(quest.id)
	);
	...
	questCache = await mongo_questCache_update(userID, {
		...
		$inc: {
			balance:
				questCache.temp?.balance || questCache.temp?.balance === 0
					? clampPos(userData.balance - questCache.temp.balance)
					: 0,

			ribbons:
				questCache.temp?.ribbons || questCache.temp?.ribbons === 0
					? clampPos(userData.ribbons - questCache.temp.ribbons)
					: 0,

			cards_in_inventory:
				questCache.temp?.cards_in_inventory || questCache.temp?.cards_in_inventory === 0
					? clampPos(userData.card_inventory.length - questCache.temp.cards_in_inventory)
					: 0
		},
		...
		$set: {
			quests_in_progress: userActiveQuests.map(({ id }) => id),
	...
	let questProgress = userActiveQuests.map(({ id }) =>
		quest_getProgress(id, userData, questCache)
	);
	...
	return {
		...
		quests_complete: questProgress
			.filter(progress => progress.complete)
			.map(progress => progress.quest)
	};
}
...
module.exports = {
	...
	user: {
		...
		questsChanged: mongo_user_questsChanged
	},
